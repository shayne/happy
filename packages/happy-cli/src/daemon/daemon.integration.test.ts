...
describe.skipIf(!await isServerHealthy())('Daemon Integration Tests', { timeout: 20_000 }, () => {
   ...
   * 
   * This simulates what happens during `npm upgrade happy-codex`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/happy-codex/ with NEW version files
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when happy-codex is uninstalled
